...
select
    ...
    cast(regexp_split_to_table(trainnlls, ':') as float) as trainnlls
from 
    %(TABLE)s
where 
    tstamps!='' and trainnlls!='';

CREATE OR REPLACE TEMP view estopper_view as
select 
    id, max(trainnlls) as score
from 
    ll_view
where
    tstamps <= :tstamp
...
select
    ...
    count(ll.trainnlls) as cnt,
    avg(ll.trainnlls) as trainnlls, 
    stddev(ll.trainnlls)/sqrt(count(ll.trainnlls)) as trainnlls_std
from
    %(TABLE)s, ll_view as ll, estopper_view
where
    ll.trainnlls=estopper_view.score and
...
select
    rbm_nbeta, max(trainnlls) as max_trainnlls
from 
    avg_seed_view
group by
    rbm_nbeta;
...
select 
    ...
    trainnlls, trainnlls_std
from
    avg_seed_view as as_view, 
    best_valid_view as bv_view
where
    bv_view.max_trainnlls = as_view.trainnlls and
...
for tstamp in tstamps:
    ...
    for row in rp.fetchall():
        y_mean[row['rbm_nbeta']].append(row['trainnlls'])
        if row['trainnlls_std']:
            y_std[row['rbm_nbeta']].append(row['trainnlls_std'])
        else:
